      ...
      if(film != undefined) {
        return (
          <ScrollView style={styles.scrollview_container}>
            ...
            <Text style={styles.default_text}>Budget : {numeral(film.title).format('0,0[.]00 $')}</Text>
            <Text style={styles.default_text}>Genre(s) : {film.genres.map(genre => genre.name).join(" / ")}</Text>
            <Text style={styles.default_text}>Companie(e) : {film.production_companies.map(company => company.name).join(" / ")}</Text>
          </ScrollView>
        )
      }
    }
